...
class MovementMap extends Component {
  ...
  componentDidMount() {
    this.setState({ windowheight: window.innerHeight + 'px', windowwidth: window.innerWidth + 'px'})
  	...
  	console.log("COMPONENTDID MOUNT BEGINS");
...
function processSpeeds(data) {
	...
	for (var key in data){
		console.log(data[key.toString()].value[3][1])
		// console.log(str.rows[id].id);
		// console.log(str.rows[id].key);
		// console.log(str.rows[id].value);
		// console.log(str.rows[id].value[3][1]);
		let lat1 = data[key.toString()].value[3][1];
		let lng1 = data[key.toString()].value[3][0];
		let lat2 = data[key.toString()].value[4][1];
		let lng2 = data[key.toString()].value[4][0];
		...
		console.log("pushed a polyline");
		console.log(polylines.slice(-1)[0]);
	}
	console.log("this is the final polylines")
